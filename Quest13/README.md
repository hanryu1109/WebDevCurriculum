...
- GraphQL API는 무엇인가요? REST의 어떤 단점을 보완해 주나요?
  ...
  > - Over-fetching 및 Under-fetching 문제 해결
  > - 쉬운 버전 관리
  > - 강력한 타입 시스템
  > - 단일 요청
  > - RESTful API 처럼 여러 엔드포인트와 요청 메소드를 조합하여 요청하는 것과 달리, GraphQL API를 사용하게 되면 클라이언트는 필요한 데이터 필드만 정확히 요청할 수 있으며 여러 엔드포인트를 호출하지 않고도 한 번의 요청(단일 API 호출)으로 모든 데이터를 가져올 수 있습니다.
  > - GraphQL 은 Query, Mutation, Subscription 타입을 통해 클라이언트의 요청을 명시합니다.
  > - GraphQL API는 대규모 데이터 요청 및 다양한 클라이언트 플랫폼에서 사용됩니다.
- GraphQL 스키마는 어떤 역할을 하며 어떤 식으로 정의되나요?

  > - GraphQL 스키마는 GraphQL API가 제공하는 데이터의 형식과 구조를 정의하는 것입니다.
  > - 일반적으로 `.graphql` 확장자를 가진 파일에 정의됩니다. 이 파일 안에서 GraphQL 타입과 필드를 정의할 수 있습니다.
  > - `schema.graphql` 이라는 파일 내에 스키마를 작성하게 되며 사용할 Query(데이터를 가져오기 위한 필드)나 Mutation(데이터를 생성, 수정, 삭제하기 위한 필드를 가집니다), type 등을 정의하는데, 즉 사용할 데이터들에 대한 설명을 스키마에 작성한다고 생각하면 됩니다.
...
- GraphQL 리졸버는 어떤 역할을 하며 어떤 식으로 정의되나요?
  ...
  > - GraphQL 리졸버는 GraphQL API에서 클라이언트의 요청에 대해 어떻게 응답할지를 결정하는 함수입니다. 리졸버는 각각의 필드에 대한 데이터를 가져오는 책임을 가지며, 서버에서 데이터를 조회, 계산 또는 다른 작업을 수행한 다음 클라이언트에게 반환합니다.
  > - 사용할 Query나 Mutation은 반드시 type Query, type Mutation 내부에 작성해야 하는 규칙을 지켜야 합니다.
  > - 그런데 정의만 한다고 해서 데이터의 읽기, 쓰기, 업데이트 등이 알아서 되지는 않습니다. 데이터를 읽어오거나, 데이터를 추가하거나 삭제 등의 기능을 구현해야 합니다. 그 기능 구현을 리졸버에 작성합니다. resolver 란 쉽게 말해 쿼리를 해결하는 것을 말합니다.
...
- GraphQL 리졸버의 성능 향상을 위한 DataLoader는 무엇이고 어떻게 쓰나요?
  ...
  > - DataLoader는 데이터 베이스 또는 웹 서비스와 같은 다양한 원격 데이터 소스 요청을 일괄 처리 또는 캐싱을 통해서 그에 대한 비용을 줄이고 쿼리의 실행속도를 높여주는 유틸리티입니다. 즉 성능 최적화를 위한 도구입니다.
...
- 클라이언트 상에서 GraphQL 요청을 보내려면 어떻게 해야 할까요?
  ...
  > - 일반적으로 클라이언트는 GraphQL 요청을 보내기 위해 HTTP 클라이언트 라이브러리를 사용합니다. 대표적인 예로는 Axios, Fetch API, Apollo Client 등이 있습니다.
...
- Apollo 프레임워크(서버/클라이언트)의 장점은 무엇일까요?
  ...
  > - Apollo Client는 GraphQL API와의 통신 및 상태 관리를 담당합니다. Apollo Client를 사용하면 GraphQL 쿼리를 작성하기 위해 gql 함수를 사용할 수 있습니다. 이후에 query 메소드를 사용하여 GraphQL 쿼리를 실행합니다.
- Apollo Client를 쓰지 않고 Vanilla JavaScript로 GraphQL 요청을 보내려면 어떻게 해야 할까요?

  > - `fetch()` 메소드를 사용하여 요청을 보낼 수 있습니다. 일반적으로는 `fetch()`를 사용하여 REST API 요청을 보내는 것과 비슷한 방식으로 GraphQL 요청을 보낼 수 있습니다.
...
- GraphQL 기반의 API를 만들 때 에러처리와 HTTP 상태코드 등은 어떻게 하는게 좋을까요?
  ...
  > - HTTP 상태 코드를 사용하여 일반적인 HTTP 에러를 나타냅니다. (200: 성공, 400: 잘못된 요청, 500: 서버에서 처리중 에러)
  > - JSON 응답에서 errors 필드(보통 GraphQL 스키마에는 errors 필드가 배열 형태로 정의되어 있습니다.)를 사용하여 GraphQL 에러를 나타냅니다
